...
public class ComboStatusHandler implements IStatusEffectHandler {
	@Override
	public void handleStatusEffects(ServerPlayer player, StatsData data) {
		if (!data.getCooldowns().hasCooldown(Cooldowns.COMBO_ATTACK_CD)) {
			player.removeEffect(MainEffects.COMBO_CD.get());
		}
	}
